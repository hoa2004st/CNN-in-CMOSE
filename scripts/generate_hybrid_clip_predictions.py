...
The naive pipeline (``src.feature_analysis.run_domain_shift_analysis``) writes
...
(``scripts/evaluate_hybrid_full_matrix.py``) only stores aggregate metrics, so
...
Usage:
    python scripts/generate_hybrid_clip_predictions.py
    python scripts/generate_hybrid_clip_predictions.py --test_set cmose_test
    python scripts/generate_hybrid_clip_predictions.py --train_groups cmose
"""
