...
def plot_stacked_bar_datasets(
    ...
    *,
    ...
    handles = [plt.Rectangle((0, 0), 1, 1, fc=CLASS_COLORS[cls]) for cls in CLASS_ORDER]
    ax.legend(handles, CLASS_ORDER, loc="upper right", fontsize=8, title="Class", title_fontsize=8)
    fig.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def plot_all_splits_combined(
    ...
    *,
    ...
    handles = [plt.Rectangle((0, 0), 1, 1, fc=CLASS_COLORS[cls]) for cls in CLASS_ORDER]
    fig.legend(handles, CLASS_ORDER, loc="lower center", ncol=4, fontsize=9,
