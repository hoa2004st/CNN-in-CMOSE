...
def _annotated_heatmap(ax, matrix, *, vmin, vmax, cmap, fmt="{:.2f}", fontsize=7,
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            ...
            texts.append(ax.text(j, i, fmt.format(value), ha="center", va="center",
                                 fontsize=fontsize, color="white" if dark else "black"))
...
def _fig_metric_correlation(frame, name: str, directory: Path | None = None) -> Path:
...
def fig_metric_correlation_base(directory: Path | None = None) -> Path:
    ...
    return _fig_metric_correlation(cell, "metric_correlation_base", directory)
