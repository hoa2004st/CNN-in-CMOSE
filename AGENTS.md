...
- **Math / equation style (see `documents/thesis/MATH_STYLE.md` for the full rules + symbol glossary).** Multi-letter operators use `\operatorname{...}` (Attention, softmax, Acc, MacroAcc, MAE), never `\mathrm`. Loss/metric subscripts use `\text{...}` (`\mathcal{L}_{\text{CE}}`, `\operatorname{Acc}_{\text{oracle}}`, `\lambda_{\text{aux}}`), never `\mathrm` and never mixed. Transpose is `^\top`; indicator is `\mathbf{1}[\cdot]`; combinatorial counts use `\binom{n}{2}`; constants discussed in prose get a named symbol (e.g. `\lambda_{\text{aux}}=0.2`), not a bare number. The hand-edit copy of every equation is `documents/thesis/equations_review.tex`; edit there, then propagate back to the chapter `.tex` files.
